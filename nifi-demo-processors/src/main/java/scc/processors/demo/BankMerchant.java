...
public class BankMerchant extends View {
    ...
    private void insertRow(String trans_year, int bank_id, String bank_name, int merch_id, String merch_name, double trans_amt) throws KuduException {
        ...
        while (scanner.hasMoreRows()){
            ...
            while (results.hasNext()){
                        ...
                        && result.getInt("MERCH_ID") == merch_id)
                {
                    ...
                    update.getRow().addDouble("TRANS_AMT", result.getDouble("TRANS_AMT") + trans_amt);
            ...
            insert.getRow().addDouble("TRANS_AMT", trans_amt);
            session.apply(insert);
            session.close();
        }
    }

    private void deleteRow(String trans_year, int bank_id, int merch_id, double trans_amt) throws Exception {
        KuduTable table = kuduClient.openTable(kuduTableName);
        KuduScanner scanner = kuduClient.newScannerBuilder(table).build();
        KuduSession session = kuduClient.newSession();
        while (scanner.hasMoreRows()){
    ...
    public void handleInsertion(FlowFile flowFile) throws Exception {
        ...
        double trans_amt = Double.parseDouble(new_values[1]);
        String trans_year = Integer.toString(formatter.parse(new_values[6]).getYear());
